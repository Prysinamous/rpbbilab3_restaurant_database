...
try {
  if(restaurants.length != 0){
    ...
    res.send(JSON.stringify({status: false, message: "Nothing here bub!"}))
}
  } catch (err) {
    res.status(500).send(err);
  }
...
app.get('restaurants?sortBy=ASC', async (req, res) => {
  ...
  restaurants = await restModel.find({}).sort({'restaurant_id' : req.query.sortBy.toLowerCase()}) //default ascending
